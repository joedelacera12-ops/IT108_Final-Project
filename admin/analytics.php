...
$total_sales = $pdo->query('SELECT SUM(total) FROM orders WHERE status = \'delivered\'')->fetchColumn();
$total_orders = $pdo->query('SELECT COUNT(*) FROM orders')->fetchColumn();
$total_users = $pdo->query('SELECT COUNT(*) FROM users')->fetchColumn();
$top_products = $pdo->query('SELECT p.name, SUM(oi.quantity) as total_quantity FROM order_items oi JOIN products p ON oi.product_id = p.id GROUP BY p.name ORDER BY total_quantity DESC LIMIT 5')->fetchAll();
