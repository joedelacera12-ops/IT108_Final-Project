...
try {
    ...
    if ($viewCheck->rowCount() > 0) {
        $sellerStmt = $pdo->prepare("SELECT * FROM v_seller_performance ORDER BY total_income DESC LIMIT 10");
        $sellerStmt->execute();
...
try {
    // Check if v_delivery_partner_performance view exists
    $viewCheck = $pdo->prepare("SHOW TABLES LIKE 'v_delivery_partner_performance'");
    $viewCheck->execute();
    if ($viewCheck->rowCount() > 0) {
        $deliveryStmt = $pdo->prepare("SELECT * FROM v_delivery_partner_performance ORDER BY completed_deliveries DESC LIMIT 10");
        $deliveryStmt->execute();
        $topDeliveryPartners = $deliveryStmt ? $deliveryStmt->fetchAll() : [];
    } else {
        // Fallback query if view doesn't exist
    ...
    error_log("Error fetching delivery partner performance: " . $e->getMessage());
    $topDeliveryPartners = [];
}
...
<div class="container-fluid py-2">
    ...
    <script>
        ...
        const months = monthlyData.map(item => item.month || 'N/A');
        const incomeData = monthlyData.map(item => parseFloat(item.total_income || 0));
        const ordersData = monthlyData.map(item => parseInt(item.total_orders || 0));
        ...
        const categoryNames = categoryData.map(item => item.category_name || 'N/A');
        const categoryIncomes = categoryData.map(item => parseFloat(item.total_income || 0));
        ...
        // Seller Income Chart
        const sellerData = <?php echo json_encode($topSellers); ?>;
        const sellerNames = sellerData.map(item => (item.seller_name || (item.first_name || 'N/A') + ' ' + (item.last_name || '')));
        const sellerIncomes = sellerData.map(item => parseFloat(item.total_income || 0));

        const sellerIncomeCtx = document.getElementById('sellerIncomeChart').getContext('2d');
        new Chart(sellerIncomeCtx, {
            type: 'bar',
            data: {
                labels: sellerNames,
                datasets: [{
        ...
        const userMonths = userGrowthData.map(item => item.month || 'N/A');
        const buyerCounts = userGrowthData.map(item => parseInt(item.buyer_count || 0));
        const sellerCounts = userGrowthData.map(item => parseInt(item.seller_count || 0));
        ...
        const methodNames = paymentMethodsData.map(item => item.payment_method || 'N/A');
        const methodAmounts = paymentMethodsData.map(item => parseFloat(item.total_amount || 0));
        ...
        const marketIndicatorsData = <?php echo json_encode($marketIndicators); ?>;
        const indicatorNames = marketIndicatorsData.map(item => item.category_name || 'N/A');
        const indicatorValues = marketIndicatorsData.map(item => parseFloat(item.avg_time_to_sale || 0));

        const marketCtx = document.getElementById('marketIndicatorsChart').getContext('2d');
        new Chart(marketCtx, {
            type: 'bar',
            data: {
                labels: indicatorNames,
                datasets: [{
                    label: 'Avg Time to Sale (Days)',
        ...
        // Seller Type Chart
        const sellerTypesData = <?php echo json_encode($sellerTypes); ?>;
        const sellerTypeNames = sellerTypesData.map(item => item.seller_type || 'N/A');
        const sellerTypeCounts = sellerTypesData.map(item => parseInt(item.count || 0));

        const sellerTypeCtx = document.getElementById('sellerTypeChart').getContext('2d');
        new Chart(sellerTypeCtx, {
            type: 'pie',
            data: {
                labels: sellerTypeNames,
                datasets: [{
                    data: sellerTypeCounts,
                    backgroundColor: [
                        '#0d6efd',
                        '#28a745',
                        '#ffc107',
