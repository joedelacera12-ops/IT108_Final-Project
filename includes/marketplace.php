<?php
// Simple, embeddable marketplace view for dashboards.
// Expects `current_user()` and `get_db()` to be available via includes/auth.php and includes/db.php
...
// Check if this is being viewed by an admin
$is_admin_view = (isset($user['role']) && $user['role'] === 'admin');

// Handle regular page request
$search_term = $_GET['search'] ?? '';

// Fetch products with seller name where possible
// For admin view, show all products; for regular users, only show published products
try {
  if ($is_admin_view) {
    // Admin can see all products regardless of status
    if ($search_term) {
      $stmt = $pdo->prepare("SELECT p.id, p.name, p.price, p.stock, p.seller_id, p.description, p.status, u.first_name, u.last_name, pi.image_path, c.name as category_name FROM products p LEFT JOIN users u ON p.seller_id = u.id LEFT JOIN product_images pi ON p.id = pi.product_id AND pi.is_primary = 1 LEFT JOIN categories c ON p.category_id = c.id WHERE (p.name LIKE ? OR p.description LIKE ? OR CAST(p.price AS CHAR) LIKE ? OR c.name LIKE ?) ORDER BY p.created_at DESC LIMIT 100");
      $stmt->execute(["%$search_term%", "%$search_term%", "%$search_term%", "%$search_term%"]);
    } else {
      $stmt = $pdo->query("SELECT p.id, p.name, p.price, p.stock, p.seller_id, p.description, p.status, u.first_name, u.last_name, pi.image_path, c.name as category_name FROM products p LEFT JOIN users u ON p.seller_id = u.id LEFT JOIN product_images pi ON p.id = pi.product_id AND pi.is_primary = 1 LEFT JOIN categories c ON p.category_id = c.id ORDER BY p.created_at DESC LIMIT 100");
    }
  } else {
...
?>
<div class="card p-3 shadow-sm mb-3" style="max-width: 100vw; overflow-x: hidden; background: var(--white);">
  <h5>Marketplace</h5>
  ...
  <form method="GET" class="mb-4" id="searchForm">
    <div class="input-group">
      ...
      <?php if ($search_term): ?>
        <a href="<?php 
          // Preserve other query parameters except search
          $query_params = [];
          ...
          echo htmlspecialchars($_SERVER['PHP_SELF'] . $query_string);
  ...
  <?php if ($search_term): ?>
    <p class="text-muted">Search results for: <strong><?php echo htmlspecialchars($search_term); ?></strong> (<?php echo count($products); ?> results)</p>
  <?php elseif ($is_admin_view): ?>
    <p class="text-muted">Admin view - showing all products in the system.</p>
  <?php else: ?>
    <p class="text-muted">Browse products from sellers on the platform.</p>
  <?php endif; ?>

  <?php if (isset($user['role']) && $user['role'] === 'seller'): ?>
    <div class="mb-3">
      <a class="btn btn-success" href="/ecommerce_farmers_fishers/seller/dashboard.php?tab=addProduct">Add Product</a>
      <small class="text-muted ms-2">Only sellers can add listings.</small>
    </div>
  <?php endif; ?>
              ...
              <div class="mt-auto d-flex justify-content-between align-items-center" style="max-width: 100vw; overflow-x: hidden;">
                <div style="max-width: 100vw; overflow-x: hidden;">
                  ...
                  <?php if ($is_admin_view): ?>
                    <div class="text-muted small">Status: <?php echo htmlspecialchars($p['status']); ?></div>
                  <?php endif; ?>
                ...
                <div class="text-end" style="max-width: 100vw; overflow-x: hidden;">
                  ...
                  <?php elseif (isset($user['role']) && $user['role'] === 'seller' && $user['id'] == $p['seller_id']): ?>
                    <a class="btn btn-sm btn-outline-secondary mt-2" href="/ecommerce_farmers_fishers/seller/products.php">Manage</a>
                  <?php elseif ($is_admin_view): ?>
                    <a class="btn btn-sm btn-outline-info mt-2" href="/ecommerce_farmers_fishers/admin/edit_product.php?id=<?php echo (int)$p['id']; ?>">Edit</a>
                  <?php else: ?>
...
function updateProductDisplay(products) {
  ...
  products.forEach(p => {
    ...
    if (typeof isAdminView !== 'undefined' && isAdminView) {
      html += `<div class="text-muted small">Status: ${escapeHtml(p.status)}</div>`;
    }
...
// Global variable to indicate admin view (set from PHP)
